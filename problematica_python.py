...
def guardar_datos_cliente(run, nombre, apellido, direccion, fono, correo, tipo, monto):
    global idcliente
    idcliente += 1
    codigo = idcliente
    deuda = 0
...
def aplicar_modificacion_cliente(id_mod, nuevos_datos):
    clientes[id_mod] = nuevos_datos
    return True

def eliminar_datos_cliente(id_elim):
    if id_elim in clientes:
        del clientes[id_elim]
        return True
    return False
